...
static apt_bool_t google_srg_stream_open(mpf_audio_stream_t *stream, mpf_codec_t *codec);
static apt_bool_t google_srg_stream_close(mpf_audio_stream_t *stream);
...
static const mpf_audio_stream_vtable_t audio_stream_vtable = {
	...
	NULL,
	NULL,
	google_srg_stream_open,
	google_srg_stream_close,
	google_srg_stream_write,
	NULL
};

/** Declaration of google_srg recognizer engine */
struct google_srg_engine_t {
	apt_consumer_task_t    *task;
};

/** Declaration of google_srg recognizer channel */
struct google_srg_channel_t {
	/** Back pointer to engine */
	google_srg_engine_t     *google_srg_engine;
	/** Engine channel base */
 ...
 *    <source name="GOOGLE-SRG-PLUGIN" priority="DEBUG" masking="NONE"/>
 */
MRCP_PLUGIN_LOG_SOURCE_IMPLEMENT(GOOGLE_SRG_PLUGIN,"GOOGLE-SRG-PLUGIN")

/** Use custom log source mark */
#define GOOGLE_SRG_LOG_MARK   APT_LOG_MARK_DECLARE(GOOGLE_SRG_PLUGIN)

/** Create google_srg recognizer engine */
MRCP_PLUGIN_DECLARE(mrcp_engine_t*) mrcp_plugin_create(apr_pool_t *pool)
{
	google_srg_engine_t *google_srg_engine = apr_palloc(pool,sizeof(google_srg_engine_t));
	apt_task_t *task;
	apt_task_vtable_t *vtable;
	apt_task_msg_pool_t *msg_pool;

	msg_pool = apt_task_msg_pool_create_dynamic(sizeof(google_srg_msg_t),pool);
	google_srg_engine->task = apt_consumer_task_create(google_srg_engine,msg_pool,pool);
	if(!google_srg_engine->task) {
		return NULL;
	}
	task = apt_consumer_task_base_get(google_srg_engine->task);
	...
	return mrcp_engine_create(
				...
				google_srg_engine,         /* object to associate */
...
static apt_bool_t google_srg_engine_destroy(mrcp_engine_t *engine)
{
	google_srg_engine_t *google_srg_engine = engine->obj;
	if(google_srg_engine->task) {
		apt_task_t *task = apt_consumer_task_base_get(google_srg_engine->task);
		apt_task_destroy(task);
		google_srg_engine->task = NULL;
	}
	return TRUE;
}
...
static mrcp_engine_channel_t* google_srg_engine_channel_create(mrcp_engine_t *engine, apr_pool_t *pool)
{
	...
	/* create google_srg recog channel */
	google_srg_channel_t *recog_channel = apr_palloc(pool,sizeof(google_srg_channel_t));
	recog_channel->google_srg_engine = engine->obj;
	/* create google_srg recog channel */
	google_srg_channel_t *recog_channel = apr_palloc(pool,sizeof(google_srg_channel_t));
	recog_channel->google_srg_engine = engine->obj;
...
static apt_bool_t google_srg_channel_open(mrcp_engine_channel_t *channel)
{
	if(channel->attribs) {
		...
		for(i=0; i<header->nelts; i++) {
			apt_log(GOOGLE_SRG_LOG_MARK,APT_PRIO_INFO,"Attrib name [%s] value [%s]",entry[i].key,entry[i].val);
		}
	}

	return google_srg_msg_signal(GOOGLE_SRG_MSG_OPEN_CHANNEL,channel,NULL);
}
...
static apt_bool_t google_srg_channel_recognize(mrcp_engine_channel_t *channel, mrcp_message_t *request, mrcp_message_t *response)
{
	...
	google_srg_channel_t *recog_channel = channel->method_obj;
	...
	if(!recog_channel->audio_out) {
		...
		if(file_path) {
			apt_log(GOOGLE_SRG_LOG_MARK,APT_PRIO_INFO,"Open Utterance Output File [%s] for Writing",file_path);
			recog_channel->audio_out = fopen(file_path,"wb");
			if(!recog_channel->audio_out) {
				apt_log(GOOGLE_SRG_LOG_MARK,APT_PRIO_WARNING,"Failed to Open Utterance Output File [%s] for Writing",file_path);
			}
		}
	}
...
static apt_bool_t google_srg_channel_timers_start(mrcp_engine_channel_t *channel, mrcp_message_t *request, mrcp_message_t *response)
{
	google_srg_channel_t *recog_channel = channel->method_obj;
	recog_channel->timers_started = TRUE;
...
static apt_bool_t google_srg_channel_request_dispatch(mrcp_engine_channel_t *channel, mrcp_message_t *request)
{
	...
	switch(request->start_line.method_id) {
		...
		case RECOGNIZER_RECOGNIZE:
			processed = google_srg_channel_recognize(channel,request,response);
			break;
		case RECOGNIZER_GET_RESULT:
			break;
		case RECOGNIZER_START_INPUT_TIMERS:
			processed = google_srg_channel_timers_start(channel,request,response);
			break;
		case RECOGNIZER_STOP:
			processed = google_srg_channel_stop(channel,request,response);
			break;
		default:
			break;
	}
...
/* Raise google_srg START-OF-INPUT event */
static apt_bool_t google_srg_start_of_input(google_srg_channel_t *recog_channel)
{
...
/* Load google_srg recognition result */
static apt_bool_t google_srg_result_load(google_srg_channel_t *recog_channel, mrcp_message_t *message)
{
	FILE *file;
	...
	/* read the google_srg result from file */
...
/* Raise google_srg RECOGNITION-COMPLETE event */
static apt_bool_t google_srg_recognition_complete(google_srg_channel_t *recog_channel, mrcp_recog_completion_cause_e cause)
{
	mrcp_recog_header_t *recog_header;
	...
	if(cause == RECOGNIZER_COMPLETION_CAUSE_SUCCESS) {
		google_srg_result_load(recog_channel,message);
	}
...
static apt_bool_t google_srg_stream_write(mpf_audio_stream_t *stream, const mpf_frame_t *frame)
{
	google_srg_channel_t *recog_channel = stream->obj;
	...
	if(recog_channel->recog_request) {
		...
		switch(det_event) {
			case MPF_DETECTOR_EVENT_ACTIVITY:
				apt_log(GOOGLE_SRG_LOG_MARK,APT_PRIO_INFO,"Detected Voice Activity " APT_SIDRES_FMT,
					MRCP_MESSAGE_SIDRES(recog_channel->recog_request));
				google_srg_start_of_input(recog_channel);
				break;
			case MPF_DETECTOR_EVENT_INACTIVITY:
				apt_log(GOOGLE_SRG_LOG_MARK,APT_PRIO_INFO,"Detected Voice Inactivity " APT_SIDRES_FMT,
					MRCP_MESSAGE_SIDRES(recog_channel->recog_request));
				google_srg_recognition_complete(recog_channel,RECOGNIZER_COMPLETION_CAUSE_SUCCESS);
				break;
			case MPF_DETECTOR_EVENT_NOINPUT:
				apt_log(GOOGLE_SRG_LOG_MARK,APT_PRIO_INFO,"Detected Noinput " APT_SIDRES_FMT,
					MRCP_MESSAGE_SIDRES(recog_channel->recog_request));
				if(recog_channel->timers_started == TRUE) {
					google_srg_recognition_complete(recog_channel,RECOGNIZER_COMPLETION_CAUSE_NO_INPUT_TIMEOUT);
				}
				break;
			default:
				break;
		...
		if(recog_channel->recog_request) {
			if((frame->type & MEDIA_FRAME_TYPE_EVENT) == MEDIA_FRAME_TYPE_EVENT) {
				if(frame->marker == MPF_MARKER_START_OF_EVENT) {
					apt_log(GOOGLE_SRG_LOG_MARK,APT_PRIO_INFO,"Detected Start of Event " APT_SIDRES_FMT " id:%d",
						MRCP_MESSAGE_SIDRES(recog_channel->recog_request),
						frame->event_frame.event_id);
				}
				else if(frame->marker == MPF_MARKER_END_OF_EVENT) {
					apt_log(GOOGLE_SRG_LOG_MARK,APT_PRIO_INFO,"Detected End of Event " APT_SIDRES_FMT " id:%d duration:%d ts",
						MRCP_MESSAGE_SIDRES(recog_channel->recog_request),
						frame->event_frame.event_id,
...
static apt_bool_t google_srg_msg_signal(google_srg_msg_type_e type, mrcp_engine_channel_t *channel, mrcp_message_t *request)
{
	...
	google_srg_channel_t *google_srg_channel = channel->method_obj;
	google_srg_engine_t *google_srg_engine = google_srg_channel->google_srg_engine;
	apt_task_t *task = apt_consumer_task_base_get(google_srg_engine->task);
	apt_task_msg_t *msg = apt_task_msg_get(task);
	if(msg) {
		google_srg_msg_t *google_srg_msg;
		msg->type = TASK_MSG_USER;
		google_srg_msg = (google_srg_msg_t*) msg->data;

		google_srg_msg->type = type;
		google_srg_msg->channel = channel;
		google_srg_msg->request = request;
...
static apt_bool_t google_srg_msg_process(apt_task_t *task, apt_task_msg_t *msg)
{
	google_srg_msg_t *google_srg_msg = (google_srg_msg_t*)msg->data;
	switch(google_srg_msg->type) {
		case GOOGLE_SRG_MSG_OPEN_CHANNEL:
			/* open channel and send asynch response */
			mrcp_engine_channel_open_respond(google_srg_msg->channel,TRUE);
			break;
		case GOOGLE_SRG_MSG_CLOSE_CHANNEL:
		{
			/* close channel, make sure there is no activity and send asynch response */
			google_srg_channel_t *recog_channel = google_srg_msg->channel->method_obj;
			if(recog_channel->audio_out) {
				fclose(recog_channel->audio_out);
				recog_channel->audio_out = NULL;
			}

			mrcp_engine_channel_close_respond(google_srg_msg->channel);
			break;
		}
		case GOOGLE_SRG_MSG_REQUEST_PROCESS:
			google_srg_channel_request_dispatch(google_srg_msg->channel,google_srg_msg->request);
			break;
		default:
			break;
	}
