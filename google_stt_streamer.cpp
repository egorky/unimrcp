...
// Note: This global function doesn't have access to the instance's log_callback_c_.
// Logging from here will use the macro's fallback fprintf or needs a passed-in callback.
std::shared_ptr<grpc::ChannelCredentials> LoadCredentialsFromFile(const std::string& path, google_stt_log_callback_t log_cb, void* user_data, const char* file_arg, int line_arg) {
    if (path.empty()) {
        ...
        if (log_cb) log_cb(GOOGLE_STT_LOG_LEVEL_INFO, file_arg, line_arg, "Attempting to use Google Default Credentials.", user_data);
    ...
    if (!creds_file.is_open()) {
        if (log_cb) {
            char buffer[256];
            snprintf(buffer, sizeof(buffer), "Failed to open credentials file: %s", path.c_str());
            log_cb(GOOGLE_STT_LOG_LEVEL_ERROR, file_arg, line_arg, buffer, user_data);
        } else {
            fprintf(stderr, "[GSS_LOG %s:%d] Failed to open credentials file: %s\n", file_arg, line_arg, path.c_str());
        }
        return nullptr;
    }
...
// Logging Macro for use within GoogleSttStreamer class methods
#define LOG_VIA_CALLBACK(level, ...) \
    do { \
        if (log_callback_c_) { \
            char buffer[1024]; \
            snprintf(buffer, sizeof(buffer), __VA_ARGS__); \
            log_callback_c_((level), __FILE__, __LINE__, buffer, user_data_c_); \
        } else { \
            fprintf(stderr, "[GSS_LOG_FALLBACK %s:%d] ", __FILE__, __LINE__); \
...
GoogleSttStreamer::GoogleSttStreamer(
    ...
    google_stt_log_callback_t log_callback,
    void* user_data)
    : credentials_path_(credentials_path),
      result_callback_c_(result_callback),
      log_callback_c_(log_callback),
      user_data_c_(user_data),
      stream_active_(false),
      reader_thread_active_(false) {

    // Pass the log_callback and user_data to LoadCredentialsFromFile
    auto channel_credentials = LoadCredentialsFromFile(credentials_path_, log_callback_c_, user_data_c_, __FILE__, __LINE__);
    if (!channel_credentials) {
        // Error already logged by LoadCredentialsFromFile if log_callback_c_ was valid there
        // Or log here if it wasn't (LoadCredentialsFromFile has its own fallback)
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_ERROR, "GoogleSttStreamer: Failed to load credentials. Client will not be functional.");
        return;
    }
...
bool GoogleSttStreamer::StartStream(
    ...
    if (stream_active_ || reader_thread_active_) {
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_WARNING, "GoogleSttStreamer::StartStream: Stream already active or reader thread running.");
        return false;
    }
    if (!speech_client_) {
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_ERROR, "GoogleSttStreamer::StartStream: Speech client not initialized (credentials issue?).");
        return false;
    }
    ...
    if (!streamer_) {
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_ERROR, "GoogleSttStreamer::StartStream: Failed to create AsyncStreamingRecognize stream.");
    ...
    if (!start_future.get()) { // .get() blocks until the start operation is complete
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_ERROR, "GoogleSttStreamer::StartStream: Failed to start the gRPC stream.");
        if (result_callback_c_) {
    ...
    // === Apply new settings ===
    if (!model.empty()) {
        streaming_config->mutable_config()->set_model(model);
    }
    streaming_config->mutable_config()->set_enable_automatic_punctuation(enable_automatic_punctuation);
    // === End new settings ===

    if (!streamer_->Write(config_request, grpc::WriteOptions()).get()) { // .get() blocks until write is complete
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_ERROR, "GoogleSttStreamer::StartStream: Failed to send initial config request.");
        if (result_callback_c_) {
...
bool GoogleSttStreamer::SendAudio(const void* buffer, int size) {
    ...
    if (!write_status) {
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_WARNING, "GoogleSttStreamer::SendAudio: Failed to write audio to stream.");
         if (result_callback_c_) {
...
bool GoogleSttStreamer::FinishStream() { // Corresponds to WritesDone
    if (!stream_active_.load() || !streamer_) {
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_WARNING, "GoogleSttStreamer::FinishStream: Stream not active or streamer not initialized.");
        return false;
    }
    ...
    if (!writes_done_status) {
        LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_WARNING, "GoogleSttStreamer::FinishStream: Failed to call WritesDone on stream.");
...
void GoogleSttStreamer::CloseStream() {
    ...
    if (streamer_) {
        ...
        if (!status.ok()) {
            LOG_VIA_CALLBACK(GOOGLE_STT_LOG_LEVEL_WARNING, "GoogleSttStreamer::CloseStream: Stream finish error: %s", status.message().c_str());
...
google_stt_streamer_handle_t google_stt_streamer_create(
    ...
    try {
        ...
        GoogleSttStreamer* streamer = new GoogleSttStreamer(cred_path_str, result_callback, log_callback, user_data);
        if (streamer && streamer->speech_client_ == nullptr) { 
            if (log_callback) {
                 log_callback(GOOGLE_STT_LOG_LEVEL_ERROR, __FILE__, __LINE__, "Speech client is null after GoogleSttStreamer creation, credentials issue?", user_data);
            } else {
                fprintf(stderr, "[GSS_LOG_FALLBACK %s:%d] Speech client is null after GoogleSttStreamer creation, credentials issue?\n", __FILE__, __LINE__);
            }
            delete streamer;
            return nullptr;
        }
        ...
        if (log_callback) {
            char buffer[256];
            snprintf(buffer, sizeof(buffer), "Exception in google_stt_streamer_create: %s", e.what());
            log_callback(GOOGLE_STT_LOG_LEVEL_ERROR, __FILE__, __LINE__, buffer, user_data);
        } else {
            fprintf(stderr, "[GSS_LOG_FALLBACK %s:%d] Exception in google_stt_streamer_create: %s\n", __FILE__, __LINE__, e.what());
        }
        return nullptr;
    } catch (...) {
        if (log_callback) {
            log_callback(GOOGLE_STT_LOG_LEVEL_ERROR, __FILE__, __LINE__, "Unknown exception in google_stt_streamer_create", user_data);
        } else {
            fprintf(stderr, "[GSS_LOG_FALLBACK %s:%d] Unknown exception in google_stt_streamer_create\n", __FILE__, __LINE__);
        }
        return nullptr;
    }
}
...
int google_stt_streamer_start(google_stt_streamer_handle_t handle, const char* language_code, int sample_rate_hz, bool interim_results, const char* model, bool enable_automatic_punctuation) {
    ...
    std::string model_str = model ? model : ""; // Handle null model string
    return streamer->StartStream(lang_str, sample_rate_hz, interim_results, model_str, enable_automatic_punctuation) ? 0 : -1;
}
