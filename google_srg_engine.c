...
// --- START: Added for C++ Log Callback ---
static apt_log_priority_e map_google_log_level_to_apt(google_stt_log_level_e level) {
    switch (level) {
        case GOOGLE_STT_LOG_LEVEL_ERROR:   return APT_PRIO_ERROR;
        case GOOGLE_STT_LOG_LEVEL_WARNING: return APT_PRIO_WARNING;
        case GOOGLE_STT_LOG_LEVEL_INFO:    return APT_PRIO_INFO;
        case GOOGLE_STT_LOG_LEVEL_DEBUG:   return APT_PRIO_DEBUG;
        default:                           return APT_PRIO_DEBUG; // Default to debug
    }
}

static void google_srg_log_handler_c(
    ...
    void* user_data) 
{
    // user_data is available if needed (it's the 'channel' pointer here), 
    // but for apt_log, GOOGLE_SRG_LOG_MARK is typically used.
    // GOOGLE_SRG_LOG_MARK should already be defined by MRCP_PLUGIN_LOG_SOURCE_IMPLEMENT.
    apt_log(GOOGLE_SRG_LOG_MARK, 
            map_google_log_level_to_apt(level), 
            "C++ [%s:%d] %s", 
            file ? file : "unknown_file", 
            line, 
            message ? message : "empty_message");
}
// --- END: Added for C++ Log Callback ---
...
static apt_bool_t google_srg_engine_config_load(google_srg_engine_t *srg_engine, mrcp_engine_config_t *config) {
    ...
    apt_log(GOOGLE_SRG_LOG_MARK, APT_PRIO_INFO, "Set Google Default Model: %s", 
srg_engine->default_model ? srg_engine->default_model : "(not set)");
...
static apt_bool_t google_srg_channel_open(mrcp_engine_channel_t *channel) {
    ...
    srg_channel->stt_streamer = google_stt_streamer_create(
        ...
        google_srg_on_stt_result,    // Result callback
        google_srg_log_handler_c,    // New Log callback
        channel                      // User data (channel pointer, can be used by both callbacks)
    );
...
static apt_bool_t google_srg_channel_recognize(mrcp_engine_channel_t *channel, mrcp_message_t *request, mrcp_message_t *response) {
    ...
    int rv = google_stt_streamer_start(
        ...
        srg_channel->current_interim_results, // Pass apt_bool_t directly
        srg_channel->current_model,
        srg_channel->current_punctuation      // Pass apt_bool_t directly
    );
